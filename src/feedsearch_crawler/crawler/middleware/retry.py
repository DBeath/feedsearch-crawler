...
class RetryMiddleware(BaseDownloaderMiddleware):
    def __init__(self, max_retries: int = 3, retry_statuses: List[int] = [429, 500, 502, 503, 504]):
    ...
    async def process_response(self, response: Response):
        if response.status_code in self.retry_statuses and response.request.retries < self.max_retries:
